// Description: Java 11 base object instance implementation for CFBam ISOCtryLang.
...
public class CFBamISOCtryLangObj
	implements ICFBamISOCtryLangObj
{
	...
	public CFBamISOCtryLangObj() {
		isNew = true;
		requiredContainerCtry = null;
		requiredParentLang = null;
	}

	public CFBamISOCtryLangObj( ICFSecSchemaObj argSchema ) {
		schema = argSchema;
		isNew = true;
		edit = null;
		requiredContainerCtry = null;
	...
	public ICFSecISOCtryLangObj realise() {
		ICFSecISOCtryLangObj retobj = ((ICFBamSchemaObj)schema).getISOCtryLangTableObj().realiseISOCtryLang(
			(ICFSecISOCtryLangObj)this );
		return( (ICFSecISOCtryLangObj)retobj );
	}

	public void forget() {
		((ICFBamSchemaObj)schema).getISOCtryLangTableObj().reallyDeepDisposeISOCtryLang( (ICFSecISOCtryLangObj)this );
	}

	public ICFSecISOCtryLangObj read() {
		ICFSecISOCtryLangObj retobj = ((ICFBamSchemaObj)schema).getISOCtryLangTableObj().readISOCtryLangByIdIdx( getPKey().getRequiredISOCtryId(),
	...
	public ICFSecISOCtryLangTableObj getISOCtryLangTable() {
		return( ((ICFBamSchemaObj)schema).getISOCtryLangTableObj() );
	}
	...
	public CFSecISOCtryLangBuff getBuff() {
		if( buff == null ) {
			if( isNew ) {
				buff = ((ICFBamSchema)schema.getBackingStore()).getFactoryISOCtryLang().newBuff();
			}
			else {
				// Read the data buff via the backing store
				buff = ((ICFBamSchema)schema.getBackingStore()).getTableISOCtryLang().readDerivedByIdIdx( ((ICFBamSchemaObj)schema).getAuthorization(),
	...
	public CFSecISOCtryLangPKey getPKey() {
		if( pKey == null ) {
			pKey = ((ICFBamSchema)schema.getBackingStore()).getFactoryISOCtryLang().newPKey();
		}
		return( pKey );
	}
	...
	public ICFSecISOCtryLangEditObj beginEdit() {
		...
		if( getIsNew() ) {
			...
			lockobj = ((ICFBamSchemaObj)schema).getISOCtryLangTableObj().lockISOCtryLang( getPKey() );
		}
		edit = ((ICFBamSchemaObj)schema).getISOCtryLangTableObj().newEditInstance( lockobj );
	...
	public ICFSecISOLangObj getRequiredParentLang( boolean forceRead ) {
		if( ( requiredParentLang == null ) || forceRead ) {
			boolean anyMissing = false;
			if( ! anyMissing ) {
				requiredParentLang = ((ICFBamSchemaObj)schema).getISOLangTableObj().readISOLangByIdIdx( getPKey().getRequiredISOLangId(), forceRead );
			}
		}
