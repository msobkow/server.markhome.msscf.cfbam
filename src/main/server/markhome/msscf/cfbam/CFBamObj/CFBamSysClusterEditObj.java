// Description: Java 11 edit object instance implementation for CFBam SysCluster.
	...
	implements ICFBamSysClusterEditObj
{
	...
	public CFBamSysClusterEditObj( ICFSecSysClusterObj argOrig ) {
		orig = argOrig;
		getBuff();
	...
	public ICFSecSysClusterObj realise() {
		...
		ICFSecSysClusterObj retobj = getSchema().getSysClusterTableObj().realiseSysCluster( (ICFBamSysClusterObj)this );
		return( retobj );
	}
	...
	public ICFSecSysClusterObj create() {
		copyBuffToOrig();
		ICFSecSysClusterObj retobj = ((ICFBamSchemaObj)getOrigAsSysCluster().getSchema()).getSysClusterTableObj().createSysCluster( getOrigAsSysCluster() );
	...
	public CFSecSysClusterBuff getBuff() {
		if( buff == null ) {
			buff = ((ICFBamSchema)getOrigAsSysCluster().getSchema().getBackingStore()).getFactorySysCluster().newBuff();
	...
	public ICFSecClusterObj getRequiredContainerCluster( boolean forceRead ) {
		if( forceRead || ( requiredContainerCluster == null ) ) {
			...
			if( ! anyMissing ) {
				ICFSecClusterObj obj = ((ICFBamSchemaObj)getOrigAsSysCluster().getSchema()).getClusterTableObj().readClusterByIdIdx( getSysClusterBuff().getRequiredClusterId() );
