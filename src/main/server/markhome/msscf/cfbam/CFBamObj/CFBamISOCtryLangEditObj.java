// Description: Java 11 edit object instance implementation for CFBam ISOCtryLang.
...
public class CFBamISOCtryLangEditObj
	implements ICFBamISOCtryLangEditObj
{
	...
	public CFBamISOCtryLangEditObj( ICFSecISOCtryLangObj argOrig ) {
		orig = argOrig;
		getBuff();
	...
	public String getClassCode() {
		return( CFBamISOCtryLangObj.CLASS_CODE );
	}
	...
	public ICFSecISOCtryLangObj realise() {
		...
		ICFSecISOCtryLangObj retobj = getSchema().getISOCtryLangTableObj().realiseISOCtryLang( (ICFBamISOCtryLangObj)this );
		return( retobj );
	}
	...
	public ICFSecISOCtryLangObj create() {
		copyBuffToOrig();
		ICFSecISOCtryLangObj retobj = ((ICFBamSchemaObj)getOrigAsISOCtryLang().getSchema()).getISOCtryLangTableObj().createISOCtryLang( getOrigAsISOCtryLang() );
	...
	public CFSecISOCtryLangBuff getBuff() {
		if( buff == null ) {
			buff = ((ICFBamSchema)getOrigAsISOCtryLang().getSchema().getBackingStore()).getFactoryISOCtryLang().newBuff();
	...
	public ICFSecISOCtryObj getRequiredContainerCtry( boolean forceRead ) {
		if( forceRead || ( requiredContainerCtry == null ) ) {
			...
			if( ! anyMissing ) {
				ICFSecISOCtryObj obj = ((ICFBamSchemaObj)getOrigAsISOCtryLang().getSchema()).getISOCtryTableObj().readISOCtryByIdIdx( getPKey().getRequiredISOCtryId() );
	...
	public ICFSecISOLangObj getRequiredParentLang( boolean forceRead ) {
		if( forceRead || ( requiredParentLang == null ) ) {
			boolean anyMissing = false;
			if( ! anyMissing ) {
				ICFSecISOLangObj obj = ((ICFBamSchemaObj)getOrigAsISOCtryLang().getSchema()).getISOLangTableObj().readISOLangByIdIdx( getPKey().getRequiredISOLangId() );
