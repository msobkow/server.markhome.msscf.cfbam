// Description: Java 11 edit object instance implementation for CFBam ISOLang.
...
public class CFBamISOLangEditObj
	implements ICFBamISOLangEditObj
{
	...
	public CFBamISOLangEditObj( ICFSecISOLangObj argOrig ) {
		orig = argOrig;
		getBuff();
	...
	public String getClassCode() {
		return( CFBamISOLangObj.CLASS_CODE );
	}
	...
	public ICFSecISOLangObj realise() {
		...
		ICFSecISOLangObj retobj = getSchema().getISOLangTableObj().realiseISOLang( (ICFBamISOLangObj)this );
		return( retobj );
	}
	...
	public ICFSecISOLangObj create() {
		copyBuffToOrig();
		ICFSecISOLangObj retobj = ((ICFBamSchemaObj)getOrigAsISOLang().getSchema()).getISOLangTableObj().createISOLang( getOrigAsISOLang() );
	...
	public CFSecISOLangBuff getBuff() {
		if( buff == null ) {
			buff = ((ICFBamSchema)getOrigAsISOLang().getSchema().getBackingStore()).getFactoryISOLang().newBuff();
	...
	public List<ICFSecISOCtryLangObj> getOptionalChildrenCtry() {
		...
		retval = ((ICFBamSchemaObj)getOrigAsISOLang().getSchema()).getISOCtryLangTableObj().readISOCtryLangByLangIdx( getPKey().getRequiredISOLangId(),
			false );
		return( retval );
	}
