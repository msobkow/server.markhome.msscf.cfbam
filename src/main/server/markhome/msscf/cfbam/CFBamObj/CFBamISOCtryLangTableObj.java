	...
	implements ICFBamISOCtryLangTableObj
{
	...
	{
		final String S_ProcName = "CFBamISOCtryLangTableObj.reallyDeepDisposeISOCtryLang() ";
		String classCode;
		if( obj == null ) {
			return;
		}
		...
		members.remove( pkey );
		CFSecISOCtryLangByCtryIdxKey keyCtryIdx = ((ICFBamSchema)schema.getBackingStore()).getFactoryISOCtryLang().newCtryIdxKey();
		keyCtryIdx.setRequiredISOCtryId( existing.getRequiredISOCtryId() );

		CFSecISOCtryLangByLangIdxKey keyLangIdx = ((ICFBamSchema)schema.getBackingStore()).getFactoryISOCtryLang().newLangIdxKey();
