// Description: Java 11 edit object instance implementation for CFBam ISOCtryCcy.
...
public class CFBamISOCtryCcyEditObj
	implements ICFBamISOCtryCcyEditObj
{
	...
	public CFBamISOCtryCcyEditObj( ICFSecISOCtryCcyObj argOrig ) {
		orig = argOrig;
		getBuff();
	...
	public String getClassCode() {
		return( CFBamISOCtryCcyObj.CLASS_CODE );
	}
	...
	public ICFSecISOCtryCcyObj realise() {
		...
		ICFSecISOCtryCcyObj retobj = getSchema().getISOCtryCcyTableObj().realiseISOCtryCcy( (ICFBamISOCtryCcyObj)this );
		return( retobj );
	}
	...
	public ICFSecISOCtryCcyObj create() {
		copyBuffToOrig();
		ICFSecISOCtryCcyObj retobj = ((ICFBamSchemaObj)getOrigAsISOCtryCcy().getSchema()).getISOCtryCcyTableObj().createISOCtryCcy( getOrigAsISOCtryCcy() );
	...
	public CFSecISOCtryCcyBuff getBuff() {
		if( buff == null ) {
			buff = ((ICFBamSchema)getOrigAsISOCtryCcy().getSchema().getBackingStore()).getFactoryISOCtryCcy().newBuff();
	...
	public ICFSecISOCtryObj getRequiredContainerCtry( boolean forceRead ) {
		if( forceRead || ( requiredContainerCtry == null ) ) {
			...
			if( ! anyMissing ) {
				ICFSecISOCtryObj obj = ((ICFBamSchemaObj)getOrigAsISOCtryCcy().getSchema()).getISOCtryTableObj().readISOCtryByIdIdx( getPKey().getRequiredISOCtryId() );
	...
	public ICFSecISOCcyObj getRequiredParentCcy( boolean forceRead ) {
		if( forceRead || ( requiredParentCcy == null ) ) {
			boolean anyMissing = false;
			if( ! anyMissing ) {
				ICFSecISOCcyObj obj = ((ICFBamSchemaObj)getOrigAsISOCtryCcy().getSchema()).getISOCcyTableObj().readISOCcyByIdIdx( getPKey().getRequiredISOCcyId() );
