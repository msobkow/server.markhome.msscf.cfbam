// Description: Java 11 edit object instance implementation for CFBam SecApp.
	...
	implements ICFBamSecAppEditObj
{
	...
	public CFBamSecAppEditObj( ICFSecSecAppObj argOrig ) {
		orig = argOrig;
		getBuff();
	...
	public ICFSecSecAppObj realise() {
		...
		ICFSecSecAppObj retobj = getSchema().getSecAppTableObj().realiseSecApp( (ICFBamSecAppObj)this );
		return( retobj );
	}
	...
	public ICFSecSecAppObj create() {
		copyBuffToOrig();
		ICFSecSecAppObj retobj = ((ICFBamSchemaObj)getOrigAsSecApp().getSchema()).getSecAppTableObj().createSecApp( getOrigAsSecApp() );
	...
	public CFSecSecAppBuff getBuff() {
		if( buff == null ) {
			buff = ((ICFBamSchema)getOrigAsSecApp().getSchema().getBackingStore()).getFactorySecApp().newBuff();
	...
	public ICFSecClusterObj getRequiredContainerCluster( boolean forceRead ) {
		if( forceRead || ( requiredContainerCluster == null ) ) {
			...
			if( ! anyMissing ) {
				ICFSecClusterObj obj = ((ICFBamSchemaObj)getOrigAsSecApp().getSchema()).getClusterTableObj().readClusterByIdIdx( getPKey().getRequiredClusterId() );
	...
	public List<ICFSecSecFormObj> getOptionalComponentsForm() {
		...
		retval = ((ICFBamSchemaObj)getOrigAsSecApp().getSchema()).getSecFormTableObj().readSecFormBySecAppIdx( getPKey().getRequiredClusterId(),
