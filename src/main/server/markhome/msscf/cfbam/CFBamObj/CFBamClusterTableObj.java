	...
	implements ICFBamClusterTableObj
{
	...
	{
		final String S_ProcName = "CFBamClusterTableObj.reallyDeepDisposeCluster() ";
		String classCode;
		if( obj == null ) {
			return;
		}
		...
		members.remove( pkey );
		CFSecClusterByUDomNameIdxKey keyUDomNameIdx = ((ICFBamSchema)schema.getBackingStore()).getFactoryCluster().newUDomNameIdxKey();
		keyUDomNameIdx.setRequiredFullDomName( existing.getRequiredFullDomName() );

		CFSecClusterByUDescrIdxKey keyUDescrIdx = ((ICFBamSchema)schema.getBackingStore()).getFactoryCluster().newUDescrIdxKey();
