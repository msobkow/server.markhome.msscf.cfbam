// Description: Java 11 edit object instance implementation for CFBam ServiceType.
	...
	implements ICFBamServiceTypeEditObj
{
	...
	public CFBamServiceTypeEditObj( ICFSecServiceTypeObj argOrig ) {
		orig = argOrig;
		getBuff();
	...
	public ICFSecServiceTypeObj realise() {
		...
		ICFSecServiceTypeObj retobj = getSchema().getServiceTypeTableObj().realiseServiceType( (ICFBamServiceTypeObj)this );
		return( retobj );
	}
	...
	public ICFSecServiceTypeObj create() {
		copyBuffToOrig();
		ICFSecServiceTypeObj retobj = ((ICFBamSchemaObj)getOrigAsServiceType().getSchema()).getServiceTypeTableObj().createServiceType( getOrigAsServiceType() );
	...
	public CFSecServiceTypeBuff getBuff() {
		if( buff == null ) {
			buff = ((ICFBamSchema)getOrigAsServiceType().getSchema().getBackingStore()).getFactoryServiceType().newBuff();
	...
	public List<ICFSecServiceObj> getOptionalChildrenDeployed() {
		...
		retval = ((ICFBamSchemaObj)getOrigAsServiceType().getSchema()).getServiceTableObj().readServiceByTypeIdx( getPKey().getRequiredServiceTypeId(),
			false );
		return( retval );
	}
