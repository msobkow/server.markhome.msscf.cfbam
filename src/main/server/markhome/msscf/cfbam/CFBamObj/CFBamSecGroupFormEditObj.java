// Description: Java 11 edit object instance implementation for CFBam SecGroupForm.
	...
	implements ICFBamSecGroupFormEditObj
{
	...
	public CFBamSecGroupFormEditObj( ICFSecSecGroupFormObj argOrig ) {
		orig = argOrig;
		getBuff();
	...
	public ICFSecSecGroupFormObj realise() {
		...
		ICFSecSecGroupFormObj retobj = getSchema().getSecGroupFormTableObj().realiseSecGroupForm( (ICFBamSecGroupFormObj)this );
		return( retobj );
	}
	...
	public ICFSecSecGroupFormObj create() {
		copyBuffToOrig();
		ICFSecSecGroupFormObj retobj = ((ICFBamSchemaObj)getOrigAsSecGroupForm().getSchema()).getSecGroupFormTableObj().createSecGroupForm( getOrigAsSecGroupForm() );
	...
	public CFSecSecGroupFormBuff getBuff() {
		if( buff == null ) {
			buff = ((ICFBamSchema)getOrigAsSecGroupForm().getSchema().getBackingStore()).getFactorySecGroupForm().newBuff();
	...
	public ICFSecClusterObj getRequiredOwnerCluster( boolean forceRead ) {
		if( forceRead || ( requiredOwnerCluster == null ) ) {
			...
			if( ! anyMissing ) {
				ICFSecClusterObj obj = ((ICFBamSchemaObj)getOrigAsSecGroupForm().getSchema()).getClusterTableObj().readClusterByIdIdx( getPKey().getRequiredClusterId() );
	...
	public ICFSecSecGroupObj getRequiredContainerGroup( boolean forceRead ) {
		if( forceRead || ( requiredContainerGroup == null ) ) {
			boolean anyMissing = false;
			if( ! anyMissing ) {
				ICFSecSecGroupObj obj = ((ICFBamSchemaObj)getOrigAsSecGroupForm().getSchema()).getSecGroupTableObj().readSecGroupByIdIdx( getPKey().getRequiredClusterId(),
	...
	public ICFSecSecAppObj getRequiredParentApp( boolean forceRead ) {
		if( forceRead || ( requiredParentApp == null ) ) {
			boolean anyMissing = false;
			if( ! anyMissing ) {
				ICFSecSecAppObj obj = ((ICFBamSchemaObj)getOrigAsSecGroupForm().getSchema()).getSecAppTableObj().readSecAppByIdIdx( getPKey().getRequiredClusterId(),
	...
	public ICFSecSecFormObj getRequiredParentForm( boolean forceRead ) {
		if( forceRead || ( requiredParentForm == null ) ) {
			boolean anyMissing = false;
			if( ! anyMissing ) {
				ICFSecSecFormObj obj = ((ICFBamSchemaObj)getOrigAsSecGroupForm().getSchema()).getSecFormTableObj().readSecFormByIdIdx( getPKey().getRequiredClusterId(),
