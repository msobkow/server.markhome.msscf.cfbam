	...
	implements ICFBamSecDeviceTableObj
{
	...
	{
		final String S_ProcName = "CFBamSecDeviceTableObj.reallyDeepDisposeSecDevice() ";
		String classCode;
		if( obj == null ) {
			return;
		}
		...
		members.remove( pkey );
		CFSecSecDeviceByNameIdxKey keyNameIdx = ((ICFBamSchema)schema.getBackingStore()).getFactorySecDevice().newNameIdxKey();
		keyNameIdx.setRequiredSecUserId( existing.getRequiredSecUserId() );
		keyNameIdx.setRequiredDevName( existing.getRequiredDevName() );

		CFSecSecDeviceByUserIdxKey keyUserIdx = ((ICFBamSchema)schema.getBackingStore()).getFactorySecDevice().newUserIdxKey();
