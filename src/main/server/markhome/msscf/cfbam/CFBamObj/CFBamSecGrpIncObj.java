// Description: Java 11 base object instance implementation for CFBam SecGrpInc.
...
public class CFBamSecGrpIncObj
	implements ICFBamSecGrpIncObj
{
	...
	public CFBamSecGrpIncObj( ICFSecSchemaObj argSchema ) {
		schema = argSchema;
		isNew = true;
		edit = null;
		requiredOwnerCluster = null;
	...
	public ICFSecSecGrpIncObj realise() {
		ICFSecSecGrpIncObj retobj = ((ICFBamSchemaObj)schema).getSecGrpIncTableObj().realiseSecGrpInc(
			(ICFSecSecGrpIncObj)this );
		return( (ICFSecSecGrpIncObj)retobj );
	}

	public void forget() {
		((ICFBamSchemaObj)schema).getSecGrpIncTableObj().reallyDeepDisposeSecGrpInc( (ICFSecSecGrpIncObj)this );
	}

	public ICFSecSecGrpIncObj read() {
		ICFSecSecGrpIncObj retobj = ((ICFBamSchemaObj)schema).getSecGrpIncTableObj().readSecGrpIncByIdIdx( getPKey().getRequiredClusterId(),
	...
	public ICFSecSecGrpIncTableObj getSecGrpIncTable() {
		return( ((ICFBamSchemaObj)schema).getSecGrpIncTableObj() );
	}
	...
	public CFSecSecGrpIncBuff getBuff() {
		if( buff == null ) {
			if( isNew ) {
				buff = ((ICFBamSchema)schema.getBackingStore()).getFactorySecGrpInc().newBuff();
			}
			else {
				// Read the data buff via the backing store
				buff = ((ICFBamSchema)schema.getBackingStore()).getTableSecGrpInc().readDerivedByIdIdx( ((ICFBamSchemaObj)schema).getAuthorization(),
	...
	public CFSecSecGrpIncPKey getPKey() {
		if( pKey == null ) {
			pKey = ((ICFBamSchema)schema.getBackingStore()).getFactorySecGrpInc().newPKey();
		}
		return( pKey );
	}
	...
	public ICFSecSecGrpIncEditObj beginEdit() {
		...
		if( getIsNew() ) {
			...
			lockobj = ((ICFBamSchemaObj)schema).getSecGrpIncTableObj().lockSecGrpInc( getPKey() );
		}
		edit = ((ICFBamSchemaObj)schema).getSecGrpIncTableObj().newEditInstance( lockobj );
	...
	public ICFSecSecGroupObj getRequiredParentSubGroup( boolean forceRead ) {
		if( ( requiredParentSubGroup == null ) || forceRead ) {
			boolean anyMissing = false;
			if( ! anyMissing ) {
				requiredParentSubGroup = ((ICFBamSchemaObj)schema).getSecGroupTableObj().readSecGroupByIdIdx( getPKey().getRequiredClusterId(),
